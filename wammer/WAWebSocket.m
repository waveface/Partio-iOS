...
- (void) openConnectionToUrl:(NSURL*)anURL onSucces:(WAWebSocketConnectCallback)successBlock onFailure:(WAWebSocketConnectFailure)failureBlock {
  ...
  if ([anURL isEqual:self.urlForWebSocket] &&
	  (self.connectionForWebSocket.readyState == SR_CONNECTING || self.connectionForWebSocket.readyState == SR_OPEN))
	return;
 
  if (self.connectionForWebSocket.readyState == SR_CONNECTING || self.connectionForWebSocket.readyState == SR_OPEN)
	[self.connectionForWebSocket closeWithCode:WAWebSocketNormal reason:@""];
